    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect) {
        let items: Vec<ListItem> = self.todos
            ...
            .map(|todo| {
                ...
                let line = format!("{} [{}] {} {}", status, id_short, todo.title, priority_indicator);
...
impl InputForm {
    ...
    pub fn render(&self, frame: &mut Frame, area: Rect) {
        ...
        let priority_widget = Paragraph::new(priority_text)
            ...
            .block(Block::default().title("Priority (1-3)").borders(Borders::ALL));
        ...
        let due_widget = Paragraph::new(self.due_date.as_str())
            .style(due_style)
            .block(Block::default().title("Due Date (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS, optional)").borders(Borders::ALL));
        ...
        match self.current_field {
            InputField::Title => {
                let cursor_x = chunks[0].x + u16::try_from(self.title.len()).unwrap_or(u16::MAX.saturating_sub(chunks[0].x + 2)) + 1;
                frame.set_cursor_position((cursor_x, chunks[0].y + 1));
            },
            InputField::Description => {
                let cursor_x = chunks[1].x + u16::try_from(self.description.len()).unwrap_or(u16::MAX.saturating_sub(chunks[1].x + 2)) + 1;
                frame.set_cursor_position((cursor_x, chunks[1].y + 1));
            },
            InputField::Priority => {
                let cursor_x = chunks[2].x + u16::try_from(priority_text.len()).unwrap_or(u16::MAX.saturating_sub(chunks[2].x + 2)) + 1;
                frame.set_cursor_position((cursor_x, chunks[2].y + 1));
            },
            InputField::DueDate => {
                let cursor_x = chunks[3].x + u16::try_from(self.due_date.len()).unwrap_or(u16::MAX.saturating_sub(chunks[3].x + 2)) + 1;
    ...
    pub fn parse_due_date(&self) -> Result<Option<i64>, String> {
        ...
        if let Ok(dt) = NaiveDateTime::parse_from_str(&format!("{date_str} 00:00:00"), "%Y-%m-%d %H:%M:%S") {
            return Ok(Some(dt.and_utc().timestamp()));
        }
